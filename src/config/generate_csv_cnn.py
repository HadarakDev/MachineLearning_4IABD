...
l = list(zip(activation_param, optimizer_param, loss_param, epochs_param, batch_size_param,
                lr, layers_param, kernel_shape_param, pooling_param, isGray_param, isSparse_param,
                isNorm_param, dropout_param, l1_param, l2_param))
list_set = set(l) 
unique_list = (list(list_set))
df = pd.DataFrame(unique_list, 
               columns =["activation", "optimizer", "loss", "epochs", "batch-size", "learning-rate", "layers", "kernel-shape", "pooling", "isGray", "isSparse", "isNorm", "Dropout", "L1", "L2"]) 
