    ...
    for i in range(1, len(array_layers)):
        if penultimate_output is not None:
            ...
            for j in range(jumps):
                last_output = Dense(array_layers[i], activation=linear, name=f"Dense_{i}_{j}")(last_output)
...
def resnet_model_dense(X_all, Y, isTrain,  activation, optimizer, loss, epochs, batch_size, lr, isGray, save_dir, base_path, array_layers, jumps, dropout, l1, l2):
    ...
    if isTrain:
        model = create_resnet_model_dense(X, Y, True, activation, optimizer, loss, epochs, batch_size, lr, isGray,
                                          save_dir, base_path, array_layers, jumps, dropout, l1, l2)
